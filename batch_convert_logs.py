...
"""
Project: CityCard
File: batch_convert_logs.py
Description: 批量转换 nonBattleSkills.js 中的日志调用为双日志系统
使用智能模式识别技能使用日志和效果日志
...
{spaces})"""
    ...
    # 检查是否被阻挡
    is_blocked = '阻挡' in message or '护盾' in message
    private_suffix = '，但被阻挡' if is_blocked else ''
...
{spaces}  '{skill_name}',
{spaces}  `{public_msg}`,
{spaces}  `你使用了{skill_name}{private_suffix}`
{spaces})"""
